...
public class TypeChecker {
	...
	public void check(WhileFile.FunDecl fd) {
		...
		check(fd.statements, environment, fd.ret);
	}

	public void check(List<Stmt> statements, Map<String, Type> environment,
			Type returnType) {
		for (Stmt s : statements) {
			check(s, environment, returnType);
		}
	}

	public void check(Stmt stmt, Map<String, Type> environment, Type returnType) {
		if (stmt instanceof Stmt.Assign) {
			check((Stmt.Assign) stmt, environment);
		} else if (stmt instanceof Stmt.Print) {
			check((Stmt.Print) stmt, environment);
		} else if (stmt instanceof Stmt.Return) {
			check((Stmt.Return) stmt, environment, returnType);
		} else if (stmt instanceof Stmt.VariableDeclaration) {
			check((Stmt.VariableDeclaration) stmt, environment);
		} else if (stmt instanceof Expr.Invoke) {
			check((Expr.Invoke) stmt, environment);
		} else if (stmt instanceof Stmt.IfElse) {
			check((Stmt.IfElse) stmt, environment, returnType);
		} else if (stmt instanceof Stmt.For) {
			check((Stmt.For) stmt, environment, returnType);
		} else if (stmt instanceof Stmt.While) {
			check((Stmt.While) stmt, environment, returnType);
		} else {
	...
	public Type check(Expr.Binary expr, Map<String, Type> environment) {
		...
		case GTEQ:
			...
			return new Type.Bool();
		case APPEND:
			leftType = checkInstanceOf(leftType, expr.getLhs(),
					Type.List.class, Type.Strung.class);
	...
	public Type check(Expr.Constant expr, Map<String, Type> environment) {
		...
		if (constant instanceof Boolean) {
			...
			internalFailure("unknown constant encountered (" + expr + ")",
					file.filename, expr);
			return null; // dead code
		}
	}
	...
	public Type check(Expr.Unary expr, Map<String, Type> environment) {
		...
		case NOT:
			...
			return type;
		case LENGTHOF:
			checkInstanceOf(type, expr.getExpr(), Type.List.class,
					Type.Strung.class);
			return new Type.Int();
		default:
			internalFailure("unknown unary expression encountered (" + expr
	 ...
	 * @return
	 */
	public Type checkInstanceOf(Type type, SyntacticElement element,
			Class<?>... instances) {
		...
		if (t2 instanceof Type.Void) {
			...
			if (l1Fields.keySet().equals(l2Fields.keySet())) {
				for (Map.Entry<String, Type> p : l1Fields.entrySet()) {
					if (!isSubtype(p.getValue(), l2Fields.get(p.getKey()),
							element)) {
						return false;
					}
				}
	...
	/**
	 * Determine whether two given types are euivalent. Identical types are
	 * always equivalent. Furthermore, e.g. "int|null" is equivalent to
	 * "null|int".
	 * 
	 * @param t1
